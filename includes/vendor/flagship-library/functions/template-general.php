...
function flagship_get_post_navigation( $args = array() ) {
	...
	$defaults = apply_filters( 'flagship_get_post_navigation_defaults',
		array(
			...
			'prev_text'      => __( 'Previous', 'compass' ) . esc_attr( $name ),
			'next_text'      => __( 'Next', 'compass' ) . esc_attr( $name ),
			'in_same_term'   => false,
			'excluded_terms' => '',
			'taxonomy'       => 'category',
		)
...
function flagship_get_posts_navigation( $args = array() ) {
	...
	$defaults = apply_filters( 'flagship_loop_nav_defaults',
		array(
			'format'         => 'pagination',
			'prev_text'      => sprintf( '<span class="screen-reader-text">%s</span>' , __( 'Previous Page', 'compass' ) ),
			'next_text'      => sprintf( '<span class="screen-reader-text">%s</span>', __( 'Next Page', 'compass' ) ),
			'prev_link_text' => __( 'Newer Posts', 'compass' ),
			'next_link_text' => __( 'Older Posts', 'compass' ),
		)
	);
...
function flagship_get_credit_link() {
	$link = sprintf( '<a class="author-link" href="%s" title="%s">%s</a>',
		...
		__( 'Purpose-Built WordPress Theme by', 'compass' ) . ' Flagship',
		'Flagship'
	);
...
function flagship_get_theme_link() {
	$link = sprintf( '<a class="theme-link" href="%s"title="%s">%s</a>',
		...
		__( 'The Most Advanced WordPress Starter Theme Ever Created', 'compass' ),
